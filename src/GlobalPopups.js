...
function GlobalPopups ({ rentedBooks }) {
    ...
    useEffect(() => {
        ...
        const endTime = book.customer.startTime + book.customer.day * ONE_DAY_MS;
        const timeLeft = endTime - Date.now();
        const key = book.customer.startTime;

            if(
                timeLeft > 0 && timeLeft <= ONE_DAY_MS && !warnedRef.current.has(key)
            ) {
                showPopup(book);
            ...
            return(
                <div className="global-popups">
                    {popups.map( book => (
                        <WarningPopup key={book.customer.startTime} rentedDetails={book} onClose={() => hidePopup(book.customer.startTime)}>
                        </WarningPopup>
                    ))}
                </div>
    );
