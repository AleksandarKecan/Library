...
function Rented({ rentedBooks, removeRentedBook }) {
  ...
  const calculateTimeLeft = () => {
    return rentedBooks.map(rentedDetails => {
      const endTime =
        rentedDetails.customer.startTime +
        rentedDetails.customer.day * 24 * 60 * 60 * 1000;
  ...
  return (
    <div>
      <h1 className="rented-book-title">Rented Books</h1>
      <div className="rented-book-container">
          ...
          <div key={index} className="rented-book">
              ...
              {item.book ? (
                <>
                  <img
                    ...
                    alt={item.book.title}
                    width="100"></img>
                  <div className="book-text-info"></div>
            ...
            {item.customer && (
              <div className="customer-info">
                ...
                {timeLeft[index] && (
                  <p>
                    Time left: {timeLeft[index].days} Days {timeLeft[index].hours}{" "}
                    Hours {timeLeft[index].minutes} Minutes
                  </p>
                )}
              </div>
            )}
            <div className="remove-button-container">
              <button className="remove-button" onClick={() => removeRentedBook(item.customer.startTime)}>
                X Remove
              </button>
            </div>
          </div>
